...
func HTTP2CLient(cert tls.Certificate) *http.Client {
	var (
		innerHTTP2Client = &http.Client{
			Transport: &http2.Transport{
				TLSClientConfig: &tls.Config{
...
func HTTP2ClearClient() *http.Client {
	var innerHTTP2CleartextClient = &http.Client{
		Transport: &http2.Transport{
			AllowHTTP: true,
			DialTLS: func(network, addr string, cfg *tls.Config) (net.Conn, error) {
				return net.Dial(network, addr)
...
func InitTLSCert(certFile string, keyFile string) {
...
func GetHttpsClient() *http.Client {
	//return HTTP2CLient(cert)

	return innerHTTP2Client
}
func GetHttpClient() *http.Client {

	//return HTTP2ClearClient()
...
func CallAPI(cfg Configuration, request *http.Request) (*http.Response, error) {
	...
	if request.URL.Scheme == "https" {
		//return HTTP2CLient(cert).Do(request)
		return innerHTTP2Client.Do(request)
	} else if request.URL.Scheme == "http" {
		//return HTTP2ClearClient().Do(request)
...
func InterceptH2CClient() {
	gock.InterceptClient(HTTP2ClearClient())

	// gock.InterceptClient(innerHTTP2CleartextClient)
}

func RestoreH2CClient() {
	gock.RestoreClient(HTTP2ClearClient())
	// gock.InterceptClient(innerHTTP2CleartextClient)
